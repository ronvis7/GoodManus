    ...
    async def create_a2a_server(self, base_url: str) -> A2AConfig:
        ...
        try:
            async with httpx.AsyncClient(timeout=5) as client:
                ...
                # 尝试解析agent-card验证格式正确
                agent_card = response.json()
                agent_name = agent_card.get("name", "Unknown")
        except httpx.ConnectError as e:
            logger.error(f"连接A2A服务器失败 [{base_url}]: {str(e)}")
            raise ServerRequestsError(
                f"无法连接到A2A服务器 [{base_url}]，请确认：\n"
                f"1. A2A agent是否已启动\n"
                f"2. A2A agent是否绑定到 0.0.0.0:9999（而非 127.0.0.1）\n"
                f"3. 如果使用host.docker.internal，请确认Docker网络配置正确"
            )
        except httpx.TimeoutException:
            logger.error(f"连接A2A服务器超时 [{base_url}]")
            raise ServerRequestsError(f"连接A2A服务器 [{base_url}] 超时，请检查网络")
        except Exception as e:
            logger.error(f"验证A2A服务器失败 [{base_url}]: {str(e)}")
            raise ServerRequestsError(f"验证A2A服务器失败: {str(e)}")
        ...
        # 4.调用数据仓库更新
        self.app_config_repository.save(app_config)
        logger.info(f"成功添加A2A服务器: {agent_name} [{base_url}]")
