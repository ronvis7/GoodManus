...
def _normalize_step_result(data: Any) -> Dict[str, Any]:
    """规范化步骤执行结果数据格式

    AI 可能返回空字符串、None 或不完整的对象，此函数会将其转换为有效的 Step 数据格式。
    """
    # 如果数据不是字典类型，返回默认结构
    ...
    async def execute_step(self, plan: Plan, step: Step, message: Message) -> AsyncGenerator[BaseEvent, None]:
        ...
        async for event in self.invoke(query):
            ...
            if isinstance(event, ToolEvent):
                ...
                # 10.规范化步骤执行结果数据（处理 AI 返回空字符串或不完整对象的情况）
                normalized_obj = _normalize_step_result(parsed_obj)
                new_step = Step.model_validate(normalized_obj)
