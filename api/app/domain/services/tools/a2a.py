...
class A2AClientManager:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            # 3.获取A2A服务器列表（支持A2AConfig对象或dict）
            if isinstance(self._a2a_config, dict):
                a2a_servers = self._a2a_config.get("a2a_servers", [])
            else:
                a2a_servers = self._a2a_config.a2a_servers if self._a2a_config else []

            # 4.记录日志并连接所有配置的a2a服务获取卡片信息
            logger.info(f"加载{len(a2a_servers)}个A2A服务")
            await self._get_a2a_agent_cards(a2a_servers)
    ...
    async def _get_a2a_agent_cards(self, a2a_servers: list = None) -> None:
        """根据配置连接所有a2a服务器获取AgentCard信息"""
        a2a_servers = a2a_servers or []
        # 1.循环遍历所有的a2a服务
        for a2a_server_config in a2a_servers:
            try:
                # 2.支持A2AServerConfig对象或dict
                if isinstance(a2a_server_config, dict):
                    base_url = a2a_server_config.get("base_url", "")
                    server_id = a2a_server_config.get("id", "")
                    enabled = a2a_server_config.get("enabled", False)
                else:
                    base_url = a2a_server_config.base_url
                    server_id = a2a_server_config.id
                    enabled = a2a_server_config.enabled
                ...
                # 4.存储到agent_cards
                agent_card["enabled"] = enabled
                self._agent_cards[server_id] = agent_card
            except Exception as e:
                logger.warning(f"加载A2A服务失败: {str(e)}")
                continue
